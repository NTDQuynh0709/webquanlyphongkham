...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action'])) {
    ...
    try {
        ...
        if ($action === 'add') {
            ...
            $stmt = $conn->prepare("
                INSERT INTO doctors (
                    full_name, gender, phone, email, department_id, username,
                    password, created_at, profile_image, is_deleted, status
                )
                VALUES (
                    :full_name, :gender, :phone, :email, :department_id, :username,
                    :password, NOW(), :profile_image, 0, :status
                )
            ");
            $stmt->execute([
                'full_name' => $full_name,
        ...
        if ($action === 'change_status') {
            ...
            $stmt = $conn->prepare("UPDATE doctors SET status = :status WHERE doctor_id = :doctor_id AND is_deleted = 0");
            $stmt->execute(['status' => $status, 'doctor_id' => $doctor_id]);

            $response['success'] = true;
            $response['message'] = 'Đã cập nhật trạng thái bác sĩ thành công!';
            $response['doctor'] = ['doctor_id' => $doctor_id, 'status' => $status];
        }
                        ...
                        <table id="doctorsTable">
                            ...
                            <?php else: ?>
                                <?php foreach ($doctors as $d): ?>
                                    <?php $status = isset($d['status']) ? (int)$d['status'] : 1; ?>
                                    <tr
...
function selectRow(tr){
    ...
    document.getElementById('detailSub').innerHTML = `${d.specialty || 'N/A'} • ${statusTextLocal(d.status)}`;

    document.getElementById('d_id').textContent = '#' + d.doctor_id;
    document.getElementById('d_status').innerHTML = statusPill(d.status);
...
function openStatusModal(){
    ...
    document.getElementById('status_doctor_id').value = d.doctor_id;
    document.getElementById('status_select').value = String(d.status ?? 1);
...
function submitStatusChange(){
    const doctorId = document.getElementById('status_doctor_id').value;
    const status = document.getElementById('status_select').value;

    fetch('doctors.php?action=change_status', {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: `doctor_id=${encodeURIComponent(doctorId)}&status=${encodeURIComponent(status)}`
    })
    .then(r => r.json())
    .then(data => {
...
function openEditFromRow(id){
    ...
    document.getElementById('status').value = String(d.status ?? 1);
...
function updateRow(d){
    ...
    tr.children[3].innerHTML = statusPill(d.status);
