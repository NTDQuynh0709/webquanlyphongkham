<?php
/**
 * receptionist/create_appointment.php
 * Lễ tân tạo / sửa lịch khám offline
 */
...
function is_valid_time_hm($t): bool {
...
function getDoctor(PDO $conn, int $doctorId): ?array {
    $st = $conn->prepare("
        SELECT doctor_id, full_name, department_id, status, is_deleted
        FROM doctors
        WHERE doctor_id = ?
        LIMIT 1
    ");
...
function getPatient(PDO $conn, int $id): ?array {
    $st = $conn->prepare("
        SELECT patient_id, full_name, phone, date_of_birth, gender, address, is_deleted
        FROM patients
        WHERE patient_id = ?
        LIMIT 1
    ");
...
function suggest_nearest_slot(PDO $conn, int $doctorId, string $dateYmd, DateTime $wanted, ?int $excludeApptId = null): ?DateTime {
    ...
    foreach ($sessions as [$s, $e]) {
        ...
        while (add_minutes($cursor, SERVICE_MIN) <= $e) {
            if (MIN_LEAD_MIN > 0) {
                if ($cursor < add_minutes($now, MIN_LEAD_MIN)) {
...
function validateRulesOffline(PDO $conn, int $doctorId, int $departmentId, string $dateYmd, string $timeHm, ?int $editingId = null): array {
    ...
    if (!$doc || (int)$doc['status'] !== 1 || (int)$doc['is_deleted'] !== 0) {
    ...
    $now = new DateTime('now');
    if (MIN_LEAD_MIN > 0) {
        if ($apptStart < add_minutes($now, MIN_LEAD_MIN)) {
            return [false, 'Giờ khám phải sau hiện tại tối thiểu ' . MIN_LEAD_MIN . ' phút'];
        }
    } else {
        if ($apptStart < $now) {
            return [false, 'Không thể đăng ký lịch ở quá khứ'];
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (($_POST['csrf'] ?? '') !== ($_SESSION['csrf'] ?? '')) {
        j(['ok' => false, 'message' => 'CSRF sai'], 403);
    }
...
if ((int)$patient_id > 0) {
    $selectedPatient = getPatient($conn, (int)$patient_id);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
        ...
        try {
            if ($editId > 0) {
                ...
                $st->execute([
                    ...
                    in_array($status, ['pending', 'completed', 'cancelled'], true) ? $status : 'pending',
                    $editId
                ]);
    ...
    if ($patient_id > 0 && !$selectedPatient) {
        $selectedPatient = getPatient($conn, $patient_id);
    }
}
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .alert { padding: 12px 14px; border-radius: 12px; font-weight: 800; margin-top: 10px; white-space: pre-line; }
            ...
            <div class="grid">
                ...
                <?php if ($editId > 0): ?>
                    <div>
                        ...
                        <select name="status">
                            <option value="pending" <?php echo $status === 'pending' ? 'selected' : ''; ?>>Chờ khám</option>
