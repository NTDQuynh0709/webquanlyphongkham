...
function getReceptionist(PDO $conn, int $id): ?array {
    $st = $conn->prepare("
        SELECT receptionist_id, full_name, gender, phone, email, username, password, status, created_at
        FROM receptionists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if (!$errors && $email !== '') {
        $st = $conn->prepare("
            ...
            LIMIT 1
        ");
        $st->execute([$email, $receptionist_id]);
        if ($st->fetchColumn()) {
    ...
    if (!$errors && $phone !== '') {
        $st = $conn->prepare("
            ...
            LIMIT 1
        ");
        $st->execute([$phone, $receptionist_id]);
        if ($st->fetchColumn()) {
...
<div class="wrap">
    ...
    <div class="hero">
        ...
        <div class="sub">Cập nhật thông tin tài khoản lễ tân đang đăng nhập</div>
    </div>
        ...
        <form method="post" autocomplete="off" style="margin-top:12px;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <div style="margin-top:10px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                    ...
                    <div class="pill">
                        Trạng thái:
                        <b><?php echo $status === 1 ? 'Đang hoạt động' : 'Ngưng hoạt động'; ?></b>
                    </div>
